...
const DEFAULT_HOST = process.env.RAH_MCP_HOST || "0.0.0.0";
  ...
  updates: z
    .object({
      ...
      description: z
        .string()
        .optional()
        .describe("New description (overwrites existing)"),
      content: z
        .string()
        .optional()
        .describe("Content to APPEND (not replace)"),
      link: z.string().optional().describe("New link"),
      dimensions: z
        .array(z.string())
        .optional()
        .describe("New dimensions (replaces existing)"),
      metadata: z
        .record(z.any())
        .optional()
...
const getNodesInputSchema = {
  nodeIds: z
    ...
    .min(1)
    .max(10)
    .describe("List of node IDs to load"),
};
...
const createEdgeInputSchema = {
  ...
  explanation: z
    .string()
    .min(1)
    .describe("REQUIRED: Why does this connection exist? Be specific."),
};
...
const queryEdgesInputSchema = {
  nodeId: z
    ...
    .positive()
    .optional()
    .describe("Find edges connected to this node"),
...
const updateEdgeInputSchema = {
  ...
  explanation: z
    ...
    .min(1)
    .optional()
    .describe("New explanation text (will re-infer relationship type)"),
};
...
const createDimensionInputSchema = {
  ...
  isPriority: z
    .boolean()
    .optional()
    .describe("Lock dimension for auto-assignment"),
};
...
async function callRaHApi(pathname, options = {}) {
  ...
  try {
    ...
    if (!response.ok || !body || body.success === false) {
      const errorMessage =
        body?.error || `RA-H API request failed at ${pathname}`;
...
mcpServer.registerTool(
  "rah_search_nodes",
  {
    ...
    description:
      "Find existing RA-H entries that mention a topic before adding new ones.",
...
mcpServer.registerTool(
  "rah_update_node",
  {
    ...
    description:
      "Update an existing node. Content is APPENDED (not replaced). Dimensions are replaced.",
...
mcpServer.registerTool(
  ...
  async ({ nodeIds }) => {
    const uniqueIds = Array.from(
      new Set(nodeIds.filter((id) => Number.isFinite(id) && id > 0)),
    );
    if (uniqueIds.length === 0) {
      throw new McpError(
        ErrorCode.InvalidParams,
        "No valid node IDs provided.",
      );
    }
      ...
      try {
        ...
        if (result.node) {
          nodes.push({
            ...
            content: result.node.notes ?? null,
    ...
    return {
      content: [
        {
          type: "text",
          text: `Loaded ${nodes.length} of ${uniqueIds.length} nodes.`,
        },
      ],
      structuredContent: {
        count: nodes.length,
  ...
  async ({ sourceId, targetId, explanation }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Created edge from #${sourceId} to #${targetId}`,
        },
      ],
      structuredContent: {
        success: true,
        edgeId: edge?.id || 0,
        message:
          result.message || `Created edge from #${sourceId} to #${targetId}`,
      },
    };
  },
);
    ...
    return {
      ...
      structuredContent: {
        ...
        edges: edges.map((e) => ({
          ...
          weight:
            typeof e.context?.confidence === "number"
...
mcpServer.registerTool(
  "rah_update_dimension",
  {
    ...
    description:
      "Update dimension properties (rename, description, lock/unlock).",
...
mcpServer.registerTool(
  ...
  async ({ name }) => {
    const result = await callRaHApi(
      `/api/dimensions?name=${encodeURIComponent(name)}`,
      {
        method: "DELETE",
      },
    );
...
mcpServer.registerTool(
  "rah_search_embeddings",
  {
    ...
    description:
      "Search node content using semantic similarity (vector search).",
  ...
  async ({ query, limit = 10 }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Found ${results.length} semantically similar result(s).`,
        },
      ],
      structuredContent: {
...
mcpServer.registerTool(
  "rah_extract_url",
  {
    ...
    description:
      "Extract content from a webpage URL. Returns title, content, and metadata for creating nodes.",
...
mcpServer.registerTool(
  "rah_extract_youtube",
  {
    ...
    description:
      "Extract transcript from a YouTube video. Returns title, channel, transcript, and metadata.",
...
mcpServer.registerTool(
  "rah_extract_pdf",
  {
    ...
    description:
      "Extract content from a PDF file URL. Returns title, content, and metadata for creating nodes.",
...
mcpServer.registerTool(
  "rah_get_context",
  {
    ...
    description:
      "Get orientation context: hub nodes, dimensions, stats, and available guides. Call this first.",
    inputSchema: {},
    outputSchema: {
      schema: z.object({
        nodeCount: z.number(),
  ...
  async () => {
    const hubResult = await callRaHApi("/api/nodes?sortBy=edges&limit=5", {
      method: "GET",
    });
    const hubNodes = Array.isArray(hubResult.data)
      ? hubResult.data.map((n) => ({
          id: n.id,
          title: n.title,
    ...
    const dimensions = Array.isArray(dimResult.data)
      ? dimResult.data
    ...
    const guides = Array.isArray(guideResult.data)
      ? guideResult.data.map((g) => g.name).filter(Boolean)
      : [];
    ...
    try {
      const countResult = await callRaHApi("/api/nodes?limit=1", {
        method: "GET",
      });
    ...
    return {
      content: [
        {
          type: "text",
          text: `Knowledge graph: ${stats.dimensionCount} dimensions, ${hubNodes.length} hub nodes. ${guides.length} guides available.`,
        },
      ],
...
async function handleMcpRequest(req, res) {
  ...
  try {
    ...
    const message =
      error instanceof McpError ? error.message : "MCP transport failure";
...
async function getStatusSnapshot() {
  ...
  return {
    enabled: !!httpServer,
    port: httpPort,
    url: httpPort ? `http://${getLanIp()}:${httpPort}/mcp` : null,
...
async function persistStatus() {
  try {
    ...
    logger(
      `Failed to persist MCP status: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
