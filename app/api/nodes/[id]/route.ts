...
export async function PUT(
  ...
  try {
    ...
    const incomingChunk =
      typeof body.chunk === "string" ? body.chunk : undefined;
    const incomingNotes =
    ...
    if (incomingChunk !== undefined) {
      ...
      if (!trimmedIncoming) {
        ...
        // Chunk text unchanged — but if it was never embedded, still enqueue
        if (trimmedIncoming && existingNode.chunk_status !== "chunked") {
          shouldQueueEmbed = hasSufficientContent(trimmedIncoming);
        }
      }
