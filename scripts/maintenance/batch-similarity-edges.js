...
const VEC_EXT    = process.env.SQLITE_VEC_EXTENSION_PATH || './vendor/sqlite-extensions/vec0.dylib';
...
function post(url, body) {
  return new Promise((resolve, reject) => {
    ...
    const req  = mod.request(url, {
      ...
      res.on('data', c => buf += c);
      res.on('end', () => {
        if (res.statusCode >= 400) { reject(new Error(`HTTP ${res.statusCode}: ${buf}`)); return; }
...
async function main() {
  ...
  console.log(`[batch-similarity-edges] Similarity threshold: ${SIMILARITY_THRESHOLD}`);
  console.log(`[batch-similarity-edges] Top-K per node: ${TOP_K}`);
  if (DRY_RUN) console.log('[batch-similarity-edges] DRY RUN — no edges will be created\n');
  ...
  try {
    ...
    console.error(`[batch-similarity-edges] Failed to load sqlite-vec from ${vecExtPath}: ${err.message}`);
    console.error('Set SQLITE_VEC_EXTENSION_PATH in .env.local to the correct path.');
    process.exit(1);
  }
  ...
  console.log(`[batch-similarity-edges] Nodes with embeddings: ${nodes.length}, processing: ${toProcess.length}\n`);

  // Prepare queries
  const getSimilarChunks = db.prepare(`
    SELECT c.node_id, MAX((1.0 / (1.0 + v.distance))) AS similarity
    FROM vec_chunks v
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    if ((i + 1) % 100 === 0 || i === 0) {
      console.log(`[batch-similarity-edges] Progress: ${i + 1}/${toProcess.length} (created: ${created}, skipped: ${skipped}, failed: ${failed})`);
    }

    let similar;
    try {
      similar = getSimilarChunks.all(node.id, node.id, SIMILARITY_THRESHOLD, TOP_K);
    } catch (err) {
      noChunk++;
      continue;
    }
    ...
    for (const match of similar) {
      ...
      const exists = edgeExists.get(node.id, match.node_id, match.node_id, node.id);
      if (exists) { skipped++; continue; }

      if (DRY_RUN) {
        console.log(`  [dry-run] Edge: ${node.id} → ${match.node_id} (similarity: ${match.similarity.toFixed(3)})`);
        created++;
        continue;
      }
      ...
      try {
        ...
        console.error(`  [error] Edge ${node.id}→${match.node_id}: ${err.message}`);
        failed++;
      }
    }
  }
